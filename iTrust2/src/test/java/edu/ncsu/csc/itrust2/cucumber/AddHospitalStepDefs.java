...
public class AddHospitalStepDefs {
    ...
    /**
     * Delete hospital page
     *
     * @throws InterruptedException
     */
    @When ( "I navigate to the Delete Hospital page and click delete without choosing a hospital" )
    public void deleteHospitalPage () throws InterruptedException {
        ( (JavascriptExecutor) driver ).executeScript( "document.getElementById('deletehospital').click();" );
        Thread.sleep( 250 );
    }

    /**
     * Check to be sure hospital wasn't deleted
     */
    @Then ( "the hospital is successfully deleted" )
    public void checkNotDeleted () {
        final WebElement message = driver.findElement( By.name( "success" ) );
        assertTrue( !message.getText().contains( "Hospital deleted successfully" ) );
    }
}
