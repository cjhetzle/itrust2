...
public class APIOfficeVisitController extends APIController {
    ...
    public List<Diagnosis> getDiagnoses () {
        LoggerUtil.log( TransactionType.DIAGNOSIS_LIST,
                SecurityContextHolder.getContext().getAuthentication().getName(), null,
                "HCP viewed list of diagnoses" );
    ...
    /**
     * Creates and saves diagnoses
     *
     * @param df
     *            diagnosis form
     * @return response
     */
    @PostMapping ( BASE_PATH + "/diagnoses" )
    public ResponseEntity addDiagnosis ( @RequestBody final DiagnosisForm df ) {
        try {
            final Diagnosis diagnosis = new Diagnosis( df );
            if ( null != Diagnosis.getByIcdCode( diagnosis.getIcdCode() ) ) {
                return new ResponseEntity( "Diagnosis with that ICD Code already exists.", HttpStatus.CONFLICT );
            }
            diagnosis.save();
            LoggerUtil.log( TransactionType.DIAGNOSIS_ADD,
                    SecurityContextHolder.getContext().getAuthentication().getName(), null, "Admin added a diagnosis" );
    ...
    /**
     * Deletes a diagnosis from the system
     *
     * @param id
     *            the id of the diagnosis to delete
     * @return the response entity
     */
    @DeleteMapping ( BASE_PATH + "/diagnoses/{id}" )
    @PreAuthorize ( "hasRole('ROLE_ADMIN')" )
    public ResponseEntity deleteDiagnosis ( @PathVariable ( "id" ) final String id ) {
        try {
            Diagnosis.getByIcdCode( id ).delete();
            LoggerUtil.log( TransactionType.DIAGNOSIS_DELETE,
                    SecurityContextHolder.getContext().getAuthentication().getName() );
            return new ResponseEntity( HttpStatus.OK );
        }
        catch ( final Exception e ) {
            return new ResponseEntity( "Could not delete diagnosis due to " + e.getMessage(), HttpStatus.BAD_REQUEST );
        }
    }
