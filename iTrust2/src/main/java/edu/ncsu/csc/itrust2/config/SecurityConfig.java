...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure ( final HttpSecurity http ) throws Exception {
        ...
        final String[] patterns = new String[] { "/login*", "/password/*", "/api/v1/tokens", "/api/v1/tokens/*"};

        http.authorizeRequests().antMatchers( patterns ).anonymous().anyRequest().authenticated().and().formLogin()
                .loginPage( "/login" ).defaultSuccessUrl( "/" ).and().csrf().ignoringAntMatchers( "password/*" )
                .csrfTokenRepository( CookieCsrfTokenRepository.withHttpOnlyFalse() );

        /*
         * Credit to https://medium.com/spektrakel -blog/angular2-and-spring-a-
         * friend-in- security-need-is-a-friend- against-csrf-indeed-
         * 9f83eaa9ca2e and http://docs.spring.io/spring-
