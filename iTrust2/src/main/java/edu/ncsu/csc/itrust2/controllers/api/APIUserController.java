...
public class APIUserController extends APIController {
     ...
     *
     * @param cpf
     *            form with info about passwords
     * @return response
     */
    @PostMapping ( BASE_PATH + "/password" )
    public ResponseEntity changePassword ( @RequestBody final ChangePasswordForm cpf ) {
        final User self = User.getByName( SecurityContextHolder.getContext().getAuthentication().getName() );
        final String s = self.changePass( cpf.getOld(), cpf.getNewPass(), cpf.getRepeat() );
        if ( s.equals( "Success" ) ) {
            self.save(); // prolly dont need?
            LoggerUtil.log( TransactionType.PASSWORD_CHANGE,
     ...
     */
    @DeleteMapping ( BASE_PATH + "/delusers/{id}" )
    @PreAuthorize ( "hasRole('ROLE_ADMIN')" )
    public ResponseEntity deleteUsers ( @PathVariable ( "id" ) final String id ) {
        try {
            User.getByName( id ).delete();
            LoggerUtil.log( TransactionType.DELETE_USER,
                    SecurityContextHolder.getContext().getAuthentication().getName() );
            return new ResponseEntity( HttpStatus.OK );
        }
        catch ( final Exception e ) {
            return new ResponseEntity( "Could not delete user due to " + e.getMessage(), HttpStatus.BAD_REQUEST );
        }
    }
}
