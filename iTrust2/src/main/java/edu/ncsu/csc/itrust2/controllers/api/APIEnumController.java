...
public class APIEnumController extends APIController {
    ...
    /**
     * Get the blood types as strings
     *
     * @return blood types as strings
     */
    @GetMapping ( BASE_PATH + "/bloodtypestring" )
    public List<String> getBloodTypeStrings () {
        final List<BloodType> list = Arrays.asList( BloodType.values() );
        final List<String> types = new ArrayList<String>();
        for ( int i = 0; i < list.size(); i++ ) {
            types.add( list.get( i ).toString() );
        }
        return types;
    }
